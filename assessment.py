...
def is_hometown(town):
    """Returns True if town is my hometown London and False otherwise."""
...
def full_name(first_name, last_name):
    """Returns the concatenation of your first name and last name"""
...
def same_place(first_name, last_name, hometown):
    """Evaluates whether we are from the same place.

    If not, function will ask you where you are from."""

    name = full_name(first_name, last_name)
    if is_hometown(hometown):
        print "Hi %s, we\'re from the same place!" % (name)
    else:
        print raw_input("Hi %s, where are you from?" % (name))
...
def calculate_price(base_price, state, tax=0.05):
    """Returns the total cost of an item after adding tax and various state fees"""

    tax_multiplier = 1 + tax
    item_cost = base_price * tax_multiplier
    if state == "CA":
        reclying_fee = 0.03 * base_price
        item_cost = item_cost + reclying_fee
    elif state == "PA":
        highway_safety_fee = 2.00
        item_cost = item_cost + highway_safety_fee
    elif state == "MA":
        if base_price < 100:
            commonwealth_fee = 1.00
        else:
            commonwealth_fee = 3.00
        item_cost = item_cost + commonwealth_fee
    return item_cost
...
def append_everything_to_list(lst, *arg):
    """Adds each of the items in arg to the end of our input list and then returns it."""
...
def multiply_by_3(word):
    """Returns both the input word, and the word multiplied 3 times."""

    def concatenate_3_times(word):
        """Returns word multiplied 3 times."""

        return word+word+word

    return (word, concatenate_3_times(word))
###############################################################################
